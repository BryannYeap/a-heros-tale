...
public class CharacterController2D : MonoBehaviour
{
	...
	public void Move(float move, bool crouch, bool jump)
	{
		...
		if (!crouch && m_Grounded)
		{
			...
			// Disable attack
			GetComponent<PlayerMovement>().SetCombat(true);
		}
		...
		if (m_Grounded || m_AirControl)
		{
			...
			if (crouch)
			{
				...
				// Disable attack
				GetComponent<PlayerMovement>().SetCombat(false);
			} else
			{
	...
	private void Flip()
	{
		...
		// Flipping interactable icon
		GetComponent<Player>().InteractableIcon.GetComponent<SpriteRenderer>().flipX = !m_FacingRight;
	}
}